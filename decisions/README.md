...
## DEC-006: 유료 결제(토스페이먼츠) 미운영 — 세금 이슈

- **일자**: 2026-03-15
- **결정**: 유료 티켓(`PRICE_TICKET`)의 토스페이먼츠 카드 결제는 현재 운영하지 않는다. 무료 티켓과 두둥 티켓(계좌이체)만 운영한다.
- **이유**: 세금 이슈. 유료 결제 시 PG 수수료, VAT, 세금계산서 발행 등 세무 처리가 필요하며, 현재 팀에서 이를 처리할 준비가 되어있지 않다.
- **대안**: 토스페이먼츠 결제 활성화 — 코드는 완성 상태(테스트키 → 실키 교체 + 환경변수 설정만 필요). 세무 처리 체계가 갖춰지면 활성화 가능.
- **영향**: domain/order/정책.md, domain/settlement/정책.md, 유료 티켓 생성은 제휴 호스트만 가능
- **참여자**: 팀 결정
